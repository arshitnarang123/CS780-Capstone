"""
Offline trainer: Recurrent PPO (LSTM) for OBELIX — with reward shaping and logging.
Example:
  python train_ppo_lstm_shaped.py --obelix_py ./obelix.py --out weights_shaped.pth \
      --episodes 500 --max_steps 500 --difficulty 3
"""
...
def shape_reward(reward, obs_prev, obs_next, action_idx,
    ...
    first_contact_given : bool — has the one-time contact bonus been given?
    ep_step    : current step number in the episode
    """
    shaped = reward
    ...
    # 1. Moving forward while any sensor active → approaching the box
    if action_idx == 2 and any_sensor_next:
        shaped += 2.0

    # 2. One-time bonus first time any sensor fires in episode
    if any_sensor_next and not first_contact_given:
        shaped += 5.0
        first_contact_given = True

    # 3. IR active but not moving forward → penalise rotating when aligned
    if ir_next and action_idx != 2:
        shaped -= 2.0

    # 4. Aimless rotation when no sensors active
    if not any_sensor_next and action_idx in [0, 1, 3, 4]:
        shaped -= 0.3

    # 5. Stuck against wall without IR contact → wall-bumping during find phase
...
def main():
    ...
    ap.add_argument("--out",          type=str,   default="weights_ppo_lstm_shaped_logger.pth")
    ap.add_argument("--load",         type=str,   default=None)
    ap.add_argument("--run_name",     type=str,   default="ppo_lstm_shaped")
    ...
    for ep in tqdm(range(args.episodes), desc="Training"):
        ...
        for step in range(args.max_steps):
            ...
            ep_reward        += reward      # real reward — for logging and best-model
            ep_shaped_reward += shaped_r    # shaped — for monitoring only
        ...
        # ── Log episode (real reward only — not shaped) ───────────────────────
        logger.log(ep + 1, ep_reward)
        ...
        if ep_reward > best_reward:
            ...
            tqdm.write(
                f"  ✓ Ep {ep+1:4d} | New best: {best_reward:.1f}"
                f" | Shaped: {ep_shaped_reward:.1f} — saved")
    ...
    # ── Save weights, logs, plots ─────────────────────────────────────────────
    torch.save(model.state_dict(), args.out)
    print(f"\nFinal weights : {args.out}")
    print(f"Best  weights : {best_out}  (reward: {best_reward:.1f})")
