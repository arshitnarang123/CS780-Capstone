...
def main():
    ...
    ap.add_argument("--out", type=str, default="weights_ppo_lstm_v2.pth")
    ...
    env = OBELIX(max_steps=args.max_steps, difficulty=args.difficulty, seed=args.seed,scaling_factor=5,
    ...
    for ep in tqdm(range(args.episodes), desc="Training"):
        ...
        old_states = torch.tensor(np.array(buffer.states), dtype=torch.float32).unsqueeze(0) # (1, Seq_Len, 18)
