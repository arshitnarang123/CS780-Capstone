"""
Recurrent PPO — 2-layer LSTM with Layer Normalization.
Addresses gradient explosion via LayerNorm on hidden states.

Run:
  python train_ppo_lstm_layernorm.py --obelix_py ./obelix.py \
      --out weights_ln.pth --episodes 500 --max_steps 200 --difficulty 0
"""
...
class LayerNormLSTMCell(nn.Module):
    """LSTM cell with layer normalization — stabilizes hidden state magnitude."""
...
class RecurrentActorCritic(nn.Module):
    def __init__(self, in_dim=OBS_DIM, n_actions=len(ACTIONS), hidden_size=128):
        ...
        # Two stacked LayerNorm LSTM cells
    ...
    def forward(self, x, hidden):
        ...
        hidden : ((hx1, cx1), (hx2, cx2))  — two layers
        """
        (hx1, cx1), (hx2, cx2) = hidden
        # Remove batch dim from hidden for cell-wise processing
        ...
        for t in range(seq_len):
            inp = self.fc(x[:, t, :])          # (batch, 64)
            hx1, cx1 = self.lstm1(inp, hx1, cx1)
            hx2, cx2 = self.lstm2(hx1, hx2, cx2)
            outputs.append(hx2.unsqueeze(1))   # (batch, 1, hidden)
...
def main():
    ...
    ap.add_argument("--out",          type=str,   default="weights_ln.pth")
    ...
    for ep in tqdm(range(args.episodes), desc="Training"):
        ...
        for _ in range(args.ppo_epochs):
            ...
            for start in range(0, seq_len, args.chunk_size):
                ...
                logits, values, hidden_chunk = model(s_chunk, hidden_chunk)
