...
def main():
    ...
    for ep in range(args.episodes):
        ...
        for _ in range(args.max_steps):
            if random.random() < eps(steps):
                ...
                with torch.no_grad():
                    ...
                    a = int(torch.argmax(qvals[0, -1]))

            next_obs, r, done = env.step(ACTIONS[a], render=False)

            episode.append((obs, a, r, next_obs, done))
            obs = next_obs
            steps += 1
        ...
        if len(replay) > batch_size:
            ...
            for seq in batch:
                ...
                s = torch.tensor([x[0] for x in sub]).float().unsqueeze(0)
                a = torch.tensor([x[1] for x in sub]).long()
                r = torch.tensor([x[2] for x in sub]).float()
                s2 = torch.tensor([x[3] for x in sub]).float().unsqueeze(0)
    ...
    torch.save(q.state_dict(), "weights_dueling_dqn_memory_v2.pth")
