"""
Offline trainer: Recurrent PPO (LSTM) for OBELIX.
No frame stacking required. Memory is handled internally by the LSTM.

Example:
  python train_ppo_lstm.py --obelix_py ./obelix.py --out weights.pth --episodes 3000 --difficulty 3
"""
...
class RecurrentActorCritic(nn.Module):
    def __init__(self, in_dim=OBS_DIM, n_actions=len(ACTIONS), hidden_size=256):
        super().__init__()
...
def main():
    ...
    ap.add_argument("--out", type=str, default="final_weights_ppo_lstm_256_v3.pth")
    ...
    env = OBELIX(max_steps=args.max_steps, difficulty=args.difficulty, seed=args.seed,
                 scaling_factor=5, arena_size=500, wall_obstacles=True, box_speed=2)

    # ── best-model tracking ──────────────────────────────────────────────────
    best_reward = -float('inf')
    # Use a separate filename for best vs final so you always have both
    ...
    for ep in tqdm(range(args.episodes), desc="Training"):
        ...
        for step in range(args.max_steps):
            with torch.no_grad():
                state_tensor = torch.tensor(state, dtype=torch.float32).view(1, 1, OBS_DIM)
        ...
        if ep_reward > best_reward:
            ...
            tqdm.write(f"  ✓ Episode {ep+1} | New best reward: {best_reward:.1f} — saved to {best_out}")
        # ────────────────────────────────────────────────────────────────────

        if (ep + 1) % 1 == 0:
            tqdm.write(f"Episode {ep+1} | Reward: {ep_reward:.1f} | Best so far: {best_reward:.1f}")

    # Save final weights as well (useful for comparison)
    torch.save(model.state_dict(), args.out)
    print(f"\nFinal weights saved to: {args.out}")
    print(f"Best weights saved to:  {best_out}  (reward: {best_reward:.1f})")
